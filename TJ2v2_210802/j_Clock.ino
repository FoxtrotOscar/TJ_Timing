...
void goBlanking(uint8_t tOffset){                                 // writes a blanking frame on switch from 
  ...
  sendNumber(n+1);                                                // overwrite with last larger digit
}
...
void handleCount(unsigned long secCount){
  ...
  while ((millis() - secCount) % tick > 0){  // apply an 'fuzzy' offset of 2 for Uno
...
void sendSerialS(uint8_t txtColour1, uint8_t colNumber1, uint8_t lnNumber1, const char* i0, const char* i1, const char* i2){
  ...
  HC12.print(F(" "));
  HC12.print(colNumber1);          // x pos for text
  HC12.print(F(" "));
  HC12.print(lnNumber1);           // y pos
  HC12.print(F(" "));
  HC12.print('"');
  if (i0 != nullptr) HC12.print(i0);
  if (i1 != nullptr) HC12.print(i1);
  if (i2 != nullptr) HC12.print(i2);
  HC12.print('"');
  HC12.print(F("\r"));
  HC12.print(F("paint\r"));    HC12.flush();
}
...
void writeHalt(){
  ...
  HC12.print(F("paint\r"));    HC12.flush();
  i0 = "H A L T";
  sendSerialS( /*colour(R1G2O3)=*/ 3, /*column=*/ 2, /*line=*/ lnNumber, i0, 0, 0);
  goWhistle(3);
  int delayNo = 5;
    ...
    writeReady();
      ...
      if (paramStore.Details == 2){                           // Double detail?
        if (paramStore.isFinals 
          ...
          i0 = "FINALS";
          sendSerialS( /*colour(R1G2O3)=*/ 3, /*column=*/ 8, /*line=*/ lnNumber, i0, 0, 0); // Write FINALS
          }else{
            if (sE_iter%4 == 2) {
              i0 = "C D";
              sendSerialS( /*colour(R1G2O3)=*/ 3, /*column=*/ 24, /*line=*/ lnNumber, i0, 0, 0); // Write C D
            } else { 
              i0 = "A B";
              sendSerialS( /*colour(R1G2O3)=*/ 3, /*column=*/ 24, /*line=*/ lnNumber, i0, 0, 0); // Write A B
        }
      }
    }
  } else {
...
void writeReady(void){ 
  ...
  i0 = (paramStore.notFlint || sEcount < 1) ? "   READY" :
       sEcount < 7  ? " COLLECT " : " ";
  sendSerialS( /*colour=*/ 2, /*column=*/ 0, /*line=*/ 15, i0,0,0);
  ...
  sendSerialS( /*colour=*/ orange, /*column=*/ 1, /*line=*/ 30, i0,i1,0);          //30
...
void doBarCount(uint8_t archerIndex){                     //set to paramStore.whichArcher
  ...
  uint8_t temp = n;
  n = (paramStore.notFlint == 0) && (sEcount > 7) ?                             // If a Flint and also a Flint walkup round, 
      20 :  paramStore.walkUp;                                                  // then 20s walkup, else 10 sec
  barWidth = 5;
  rectWide = 49;
  goWhistle(2);

  while (n >= 0 && !startOver){                                                 // do bar countdown  
    ...
    if (!paramStore.isFinals && countPractice == 0 && sEcount > 1){             // if not a final and the count is >1
      ...
      i0 = ((!paramStore.notFlint) && sEcount > 7) ?                            // if Flint && >7
            n > 17 ? "ADVANCE ": "==>> "                                        // "advance" for 3 sec, then arrows from 17sec
            : "END";                                                            // else END
      i1 = ((!paramStore.notFlint) && sEcount > 7 && n <= 17 ) ? 
              flintWalk[sEcount-7] : "";                                        // if flint && 3 secs of walkup passed
      sendSerialS(  /*colour(R1G2O3)=*/ 2, 
                    /*column=*/ ((paramStore.notFlint && !paramStore.isAlternating) ? 17 : 2),
                    /*line=*/     lnNumber, i0, i1, 0);                         // Write END:
      ...
      sendSerialS( /*colour(R1G2O3)=*/ 2, /*column=*/ 0, /*line=*/ lnNumber, i0, i1, 0); 
    }
    n = doCountdownBar(n, rectWide, barWidth);                                  // returns with decremented n 
  }
...
int doCountdownBar(int n, int& rectWide, int& barWidth){        // use int& to write to the variable location
  //uint8_t temp = n;
  txtColour = n ? red : green;                                  // Numbers are red until zero reached
  lnNumber = 30;                                                // position count from 10
  colNumber = (n >= 10)? 50 : 54;                               // allocate space for double (else) single 'End' digit
  ...
  if (!startOver){
    ...
    HC12.print(lnNumber);
    HC12.print(F(" "));
    HC12.print((n < 11) ? (n*5-1) : rectWide);                  // only decrement barwidth if count reaches below 11  ** 49
    ...
    HC12.print(F("paint\r"));    HC12.flush();
    sendNumber(n--);                                            // write the number and decrement n
    do{
      if (n < 0) {
      ...
      clearFromLine(6);
        ...
        if (n <= 9) {
          ...
          HC12.print(F("line 1 "));                            //  for n values above 10
          HC12.print(rectWide);                                
          HC12.print(F(" 18 "));                               //  draw the yellow strip over the red one column wide
          HC12.print(rectWide);                                //  starting at rectwide and decrement that position
          HC12.print(F(" "));
