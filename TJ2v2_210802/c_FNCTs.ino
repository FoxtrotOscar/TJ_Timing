...
void writeInfoBigscreen(void){
  ...
  if (paramStore.isFinals && !paramStore.isAlternating) {
    ...
    if (paramStore.notFlint == 0){
      ...
      i0 = (paramStore.Details == 1 ? "Single " : "Double "); i1 = "Detail";  // either 1 OR 2
    }
    sendSerialS( /*colour(R1G2O3)=*/ 1, /*column=*/ 0, /*line=*/ lnNumber, i0, i1, 0);
  }
  delay(tick);
  
  lnNumber += 7;  // Practice?
  i0 = "Practice";
  sendSerialS( /*colour(R1G2O3)=*/ 2, /*column=*/ 0, /*line=*/ lnNumber, i0, 0, 0);
...
void writeSplash(bool scrolling){
  ...
  for (uint16_t b = 0; b <= bright; b += 10){                              // fade increment
    ...
    HC12.print(F("\r"));                                                   //  |
    HC12.print(F("paint\r"));      delay(tock); HC12.flush();
    HC12.print(F("font 16\r"));    delay(tock); HC12.flush();
    i0 = "Time";
    sendSerialS( /*colour=*/ 2, /*column=*/ 2, /*line=*/ 24, i0,0,0);
    delay(2*tock);
  ...
  if (scrolling){
    ...
    for (uint8_t iter = 0; iter < strlen(scrollChar); iter ++) {
      HC12.print(F("font 9\r"));     delay(tock); HC12.flush();             // Font for the static letter post-scroll    
      HC12.print(F("text "));      
      HC12.print(orange);      
      HC12.print(F(" "));      
...
void sendScrollW(uint16_t     scrollSpeed, 
  ...
  //char scrollChar[] = "JUDGE";
  HC12.print(F("scrollspeed "));
  HC12.print(scrollSpeed);
  HC12.print(F("\r"));    delay(tock); HC12.flush();
  ...
  for (uint8_t iter = 0; iter < strlen(scrollChar); iter ++){
    unsigned long timer = millis();
    HC12.print(F("font 17\r"));    delay(tock); HC12.flush();
    HC12.print(F("scroll "));
