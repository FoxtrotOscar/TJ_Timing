  ...
  */
  while (continueOn == 1 && !startOver) {
    if ((countPractice && sEcount == 1) || !paramStore.isAlternating ){
      ...
      n = startCounts[paramStore.startCountsIndex];
    ...
    while (n >= 0){
      writeArcher(archerIndex);
      if (!goEmergencyButton(archerIndex)) {
      writeOLED_Data(archerIndex);
      writeStopwatch(n);                                                  // Write countdown large on OLED
      if (n == startCounts[paramStore.startCountsIndex]) goWhistle(1);    // if we are at the beginning sound the GO!
        HC12.print(F("font 13\r"));
        delay(2); 
