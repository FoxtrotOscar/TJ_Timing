  ...
  if (intervalOn){
    ...
    if (lapsed < 60) {                            // when count reaches 1 min blank and post end data
      ...
      for (byte i = 0; i < 4; i++){               // flash the prepare message
        delay(tick);
        sendSerialS( /*colour=*/ 2, /*column=*/ 0, /*line=*/ 18, i0,0,0);
        delay (2*tick);
        clearMatrix();
        delay (tick);
      }
...
uint16_t goCountdownTimer(){
  if ((lapsed == paramStore.breakPeriod *60) || (lapsed %60 == 0)) { 
    ...
    i0 =  "Shooting in:";
    sendSerialS(green, /*column=*/ 2, /*line=*/ 5, i0, 0, 0);
  
    HC12.print(F("font 9\r"));    HC12.flush();
    i0 = TimeToString(--lapsed);                                // convert to hh:mm
    sendSerialS(lapsed <= 120 ? orange: green,13,25,i0,0,0);    // choose digit colour and decrement
  }
    
  HC12.print(F("font 5\r"));     HC12.flush();
  i0 = SecToString(lapsed);                                     // convert to :SS
  sendSerialS(orange,42,26,i0,0,0);                             // choose digit colour and decrement
  lapsed --;
