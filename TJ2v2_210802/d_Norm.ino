    ...
    if (!startOver) {
      ...
      if(paramStore.Details == 2 && sE_iter%2 == 1){              // now test for NEXT or HALT
        ...
        while (n >= 0){                                           // do bar countdown  ((>-1)?)
          ...
          i0 = "NEXT:";
          sendSerialS( /*colour(R1G2O3)=*/ 2, /*column=*/ 2, /*line=*/ lnNumber, i0, 0, 0);      // Write NEXT:
            lnNumber = 15; 
          if (paramStore.Details == 2){                            // Double detail?
            if (sE_iter%4 == 1) {
              i0 = "C D";
              sendSerialS( /*colour(R1G2O3)=*/ 3, /*column=*/ 44, /*line=*/ lnNumber, i0, 0, 0); // Write C D
            } else { 
              i0 = "A B";
              sendSerialS( /*colour(R1G2O3)=*/ 3, /*column=*/ 44, /*line=*/ lnNumber, i0, 0, 0); // Write A B
            }
          }
