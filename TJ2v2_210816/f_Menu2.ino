// **************************************************************************************
//                    Sergey Parshin          Menu2
// **************************************************************************************

// defines the type of the menu state
// note: difference between "enum" and "enum class" is that the latter one is a more strict in conversion to ingeters. 
// In particular if you do the following: 
//
//  enum Something { A = 1, B = 2, C = 3};
//
// then you can do:
//
//  int value = A; 
//
// or equally: 
//
//  int value = Something::A; 
//
// This will compile and work as expected, as well as: 
// 
//  Something value = A; 
//
// but if you define "Something" as: 
// 
//  enum class Something { A = 1, B = 2, C = 3};
// 
// then the following is the compiler error: 
// 
//  int value = Something::A;  // an error  
//
// And only the following is a valid syntax: 
// 
//  Something value = Something::A; 
// 
// or to get an int value, you have to do an explicit cast:
//
//  int value = (int)Something::A;  
//
// or better in C++ way:
//  
//  int value = static_cast<int>(Something::A);
// 
// So in short - if you need your enum to hold some integer values - use "enum", 
// if you need enum to just hold some named distinguished values (like states of the state machine)
//  - better way is to use "enum class" 
//

 
//enum class MenuState {
//  start, 
//  awaiting_confirmation_1, 
//  awaiting_confirmation_2, 
//  menu_root,
//  start_INTERVALS_menu, 
//  start_duration_menu,
//  start_ends_menu,
//  start_practice_menu,
//  start_details_menu,
//  start_walkup_menu,
//  start_finals_menu,
//  confirmed,
//};
//
//struct MenuContext 
//{
//  bool      displayParams     = false;
//  uint8_t   rootMenuIndex         = 0;        // currently selected menu item at root level menu
//  uint8_t   menuStartCountsIndex  = 0;        // currently selected item in the "duration" submenu
//  uint8_t   menuWalkup            = 0;        // same as above
//  uint8_t   menuMaxEnds           = 0;        // currently selected value for 'maxEnds' - unconfirmed one
//  uint8_t   menuPractice          = 0;        // same as above
//  uint8_t   menuDetails           = 0;        // same as above
//  uint8_t   menuFinals            = 0;
//  uint8_t   menuAlternating       = 0;
//  uint16_t  menuInterval          = 0;  
//};
//
//
//// each of the following functions handles particular state, and returns what must be the next state 
//
//MenuState menu_handle_start(MenuContext& ctx)
//{
//    u8x8.setFont(u8x8_font_chroma48medium8_r);
//    wipeOLED(); 
//    if (ctx.displayParams){
//        EEPROM.put(0, paramStore);              // Here the paramStore data is copied into EEPROM 
//        EEPROM.put(29, 111);                    // set flag for stored parameters
//        displayParamsOnOLED();
//    } else 
//        ctx.displayParams = true;                   // we skip displayParamsOnOLED only for the first time, 
//                        // so if some code below jumps back to start, 
//                        // we would display the whole current params again
//    u8x8.clearLine(5);
//
//    // Initial state - display confirmation request
//    writeMenuCommands();
//    
//    // return what must be the next state of the State Machine 
//    return MenuState::awaiting_confirmation_1;  
//}
//
//MenuState menu_handle_awaiting_confirmation_1(MenuContext& ctx)
//{
//  // Actually wait for user confirmation
//  // Pressing BTN1 would proceed with requesting it to be pressed 2nd time
//  // Pressing any other button will open the menu
//      
//    switch (waitButton()) 
//    {
//    case BUTTON1:
//      u8x8.setCursor(0, 5); 
//      u8x8.print("B1 Pressed      ");
//      u8x8.setCursor(0, 6);
//      u8x8.print("Confirm:  BTN[1]");
//      u8x8.setCursor(0, 7);
//      u8x8.print("Change:   BTN[2]");
//      return (continueOn == 0) ? MenuState::confirmed : MenuState::awaiting_confirmation_2;
//      
//    case BUTTON2: 
//      u8x8.fillDisplay();
//      delay(30);
//      clearFromLine(1);
//      
//      u8x8.draw2x2String(6, 2, "OK");
//      u8x8.draw2x2String(4, 5, "MENU:");
//      zeroSettings();
//      delay(700);
//      wipeOLED();
//      // go to menu_root label to display the root level of the menu
//      return MenuState::menu_root;
//
//    case BUTTON3:
//        
//      for (;;) 
//      {
//        // the only exits from this loop are two return statements for cases where user did press BTN1 or BTN4 
//        
//        wipeOLED();
//        delay(30);
//        clearFromLine(1);
//        if (paramStore.breakPeriod > 240) paramStore.breakPeriod = 240;  // catch spurious high numbers
//        u8x8.setCursor(3, 2);
//        u8x8.setFont (u8x8_font_amstrad_cpc_extended_f);
//        u8x8.print("RUN ");
//        u8x8.print(paramStore.breakPeriod);
//        u8x8.print(" min");
//        u8x8.draw2x2String(2, 4, "TIMER?");
//        u8x8.setCursor(1, 7);
//        u8x8.print("[1]Yes / No[4]");
//              
//        switch (waitButton()) {
//        case BUTTON1:
//          intervalOn = true;
//          return MenuState::start;
//        
//        case BUTTON2:
//          paramStore.breakPeriod > 231 ?  paramStore.breakPeriod = 5 : paramStore.breakPeriod++;
//          do { paramStore.breakPeriod ++; } while (paramStore.breakPeriod % 10);    
//          break;
//          
//          
//        case BUTTON3:
//          paramStore.breakPeriod <= 3 ?  paramStore.breakPeriod = 240 : paramStore.breakPeriod --;
//          break;
//          
//        case BUTTON4:
//          intervalOn = false;
//          return MenuState::start;
//        }
//      }       
//      break;
//      
//    case BUTTON4:
//      displayParamsOnOLED(); //+++++++++++++++++++++++++++++++++++++++++++++++++
//      writeSplash(false);
//      delay(2 * tick);
//      intervalOn = false;
//      started = false;
//      writeInfoBigscreen();
//      writeMenuCommands();
//      // state - unchanged  -- any other button - go and read button state again
//      break;
//  }
//  
//  // unless we have returned another state somewhere above - return the current state again, as we want to iterate here more 
//  return MenuState::awaiting_confirmation_1;
//}
//
//MenuState menu_handle_awaiting_confirmation_2(MenuContext& ctx)
//{
//    // Now - awaiting 2nd confirmation
//    switch (waitButton()) {
//    case BUTTON1:
//      //HCuOLED.clear();
//      u8x8.setCursor(0, 7); 
//      u8x8.print("     Confirmed");
//      return MenuState::confirmed;
//    
//    case BUTTON2:
//        u8x8.fillDisplay();
//        for( uint8_t r = 1; r < 8; r++ )
//        {
//            u8x8.clearLine(r);
//            delay(20);
//        } 
//        u8x8.draw2x2String(6, 2, "OK");
//        u8x8.draw2x2String(4, 5, "MENU:");
//        delay(700);
//        return MenuState::menu_root;
//    }
//    
//  return MenuState::awaiting_confirmation_2; // by default always return the current state 
//}
//
//MenuState menu_handle_menu_root(MenuContext& ctx)
//{
//    ///////////////////////////////////////////
//    ////////////// Main Menu //////////////////
//    ///////////////////////////////////////////
//    
//    // Display the root page of the menu, 0 - page index, rootSelectionIdx - current selection 
//    wipeOLED();
//    displayMenuPage(0, ctx.rootMenuIndex);
//  
//    switch (waitButton()) {      
//    case BUTTON1:
//        if (ctx.rootMenuIndex == MENU_INDEX_DURATION) {
//            // selected time duration 
//          ctx.menuStartCountsIndex = paramStore.startCountsIndex;
//          return MenuState::start_duration_menu;
//        }
//        else if (ctx.rootMenuIndex == MENU_INDEX_ENDS) {
//          ctx.menuMaxEnds = paramStore.maxEnds;
//          return MenuState::start_ends_menu;
//        }
//        else if (ctx.rootMenuIndex == MENU_INDEX_PRACTICE) {
//          ctx.menuPractice = paramStore.maxPrac ;
//          return MenuState::start_practice_menu;
//        }
//        else if (ctx.rootMenuIndex == MENU_INDEX_DETAILS) {
//          ctx.menuDetails = paramStore.Details;
//          return MenuState::start_details_menu;
//        }
//        else if (ctx.rootMenuIndex == MENU_INDEX_WALKUP) {
//          ctx.menuWalkup = paramStore.walkUp;
//          return MenuState::start_walkup_menu;
//        }
//        else if (ctx.rootMenuIndex == MENU_INDEX_FINALS) {
//          ctx.menuFinals = paramStore.isFinals;
//          ctx.menuAlternating = paramStore.isAlternating;
//          return MenuState::start_finals_menu;
//        }
//        else if (ctx.rootMenuIndex == MENU_INDEX_INTERVAL) {
//          ctx.menuInterval = (paramStore.breakPeriod > 240 ? 10 : paramStore.breakPeriod);  // catch spurious numbers
//          return MenuState::start_INTERVALS_menu;
//        }
//        break;
//  
//    case BUTTON3: 
//        // state unchanged - menu_root 
//        ctx.rootMenuIndex = (ctx.rootMenuIndex + 1) % 7; // this will increment rootMenuIndex, but if it was 6 before - it would jump back to 0
//        return MenuState::menu_root;
//  
//    case BUTTON2: 
//      // state unchanged - menu_root 
//      ctx.rootMenuIndex = (ctx.rootMenuIndex -1 + 7) % 7; // same as above but for decrement, and if it was 0 - it would jump to 6
//      return MenuState::menu_root;
//  
//    case BUTTON4:
//      return MenuState::start;
//    }
//    
//    return MenuState::menu_root; // by default always return the current state 
//}
//
//MenuState menu_handle_start_INTERVALS_menu(MenuContext& ctx)
//{
//    ////////////////////////////////////////////
//    ////////////// Intervals Menu //////////////
//    ////////////////////////////////////////////
//    wipeOLED();
//    u8x8.setCursor(0, 2);
//    u8x8.print("Timer Dur: ");
//    u8x8.print(ctx.menuInterval);
//    doButtonMenu();
//    
//    switch (waitButton())
//    {
//    case BUTTON1: 
//      paramStore.breakPeriod = ctx.menuInterval;                          // save the new val
//      return MenuState::menu_root;
//      
//    case BUTTON2: 
//      ctx.menuInterval > 231 ? ctx.menuInterval = 5 : ctx.menuInterval +=1;       // UP in 10s and not past 240
//      do { ctx.menuInterval ++; } while (ctx.menuInterval % 10);        
//      break;
//      
//    case BUTTON3: 
//      (ctx.menuInterval <= 2) ? ctx.menuInterval = 240 : ctx.menuInterval --;     // DOWN in single digits, not lower than 2
//      break;
//  
//    case BUTTON4:
//      ctx.menuInterval = paramStore.breakPeriod;                          // no change, exit
//      return MenuState::menu_root;
//    }
//    
//    return MenuState::start_INTERVALS_menu; // by default always return the current state 
//}
//
//MenuState menu_handle_start_duration_menu(MenuContext& ctx)
//{
//    ///////////////////////////////////////////
//    ////////////// Duration Menu //////////////
//    ///////////////////////////////////////////
//    wipeOLED();
//    u8x8.setCursor(0, 2);
//    u8x8.print("Start Dur: "); 
//    u8x8.print(startCounts[ctx.menuStartCountsIndex]);
//    doButtonMenu();
//    switch (waitButton())
//    {
//    case BUTTON1: 
//      paramStore.startCountsIndex = ctx.menuStartCountsIndex;
//      paramStore.notFlint = true;
//      return MenuState::menu_root;
//  
//    case BUTTON2: 
//      ctx.menuStartCountsIndex = (ctx.menuStartCountsIndex -1 + 7) % 7;   // cycle through 6 options
//      break; // state unchanged 
//  
//    case BUTTON3: 
//      ctx.menuStartCountsIndex = (ctx.menuStartCountsIndex +1) % 7;       // ditto
//      break;
//  
//    case BUTTON4:
//      ctx.menuStartCountsIndex = paramStore.startCountsIndex; 
//      return MenuState::menu_root;
//    }
//
//    return MenuState::start_duration_menu; // by default always return the current state 
//}
//
//MenuState menu_handle_start_ends_menu(MenuContext& ctx)
//{
//    ///////////////////////////////////////////
//    ////////////// Ends Menu //////////////////
//    ///////////////////////////////////////////
//    wipeOLED();
//    u8x8.setCursor(0, 2);
//    u8x8.print("Ends: "); u8x8.print(ctx.menuMaxEnds);
//    doButtonMenu();
//  
//    switch (waitButton())
//    {
//    case BUTTON1: 
//      paramStore.maxEnds = ctx.menuMaxEnds;
//      paramStore.notFlint = true;
//      sEcount = 1;
//      return MenuState::menu_root;
//  
//    case BUTTON3: 
//      if (ctx.menuMaxEnds > 1)
//        -- ctx.menuMaxEnds;
//      break;
//  
//    case BUTTON2: 
//      if (ctx.menuMaxEnds < 100)
//        ++ ctx.menuMaxEnds;
//      break;
//  
//    case BUTTON4:
//      ctx.menuMaxEnds = paramStore.maxEnds;
//      return MenuState::menu_root;
//    }
//  
//    return MenuState::start_ends_menu; // by default always return the current state 
//
//}
//
//MenuState menu_handle_start_practice_menu(MenuContext& ctx)
//{
//    ///////////////////////////////////////////
//    ////////////// Practice Menu //////////////////
//    ///////////////////////////////////////////
//    wipeOLED();
//    u8x8.setCursor(0, 2);
//    u8x8.print("Practice: "); u8x8.print(ctx.menuPractice);
//    doButtonMenu();
//  
//    switch (waitButton())
//    {
//    case BUTTON1: 
//      paramStore.maxPrac = countPractice = ctx.menuPractice;
//      return MenuState::menu_root;
//  
//    case BUTTON3: 
//      if (ctx.menuPractice > 0)
//        -- ctx.menuPractice;
//      break;
//  
//    case BUTTON2: 
//      if (ctx.menuPractice < 4)
//        ++ ctx.menuPractice;
//      break;
//  
//    case BUTTON4:
//      ctx.menuPractice = countPractice = paramStore.maxPrac;
//      return MenuState::menu_root;
//    }
//  
//    return MenuState::start_practice_menu; // by default always return the current state 
//}
//
//MenuState menu_handle_start_details_menu(MenuContext& ctx)
//{
//    ///////////////////////////////////////////
//    ////////////// Details Menu ///////////////
//    ///////////////////////////////////////////
//
//    wipeOLED();
//    u8x8.setCursor(0, 2);
//    u8x8.print("Details: "); 
//    u8x8.print(ctx.menuDetails == 2 ? "Double " : "Single ");
//    doButtonMenu();
//  
//    switch (waitButton())
//    {
//    case BUTTON1: 
//      paramStore.Details = ctx.menuDetails;
//      paramStore.notFlint = true;
//      return MenuState::menu_root;
//  
//    case BUTTON2: 
//      ctx.menuDetails = ctx.menuDetails == 2 ? 1 : 2; 
//      break;
//  
//    case BUTTON3: 
//      ctx.menuDetails = ctx.menuDetails == 2 ? 1 : 2; 
//      break;
//  
//    case BUTTON4:
//      ctx.menuDetails = paramStore.Details;
//      return MenuState::menu_root;
//    }
//  
//    return MenuState::start_details_menu; // by default always return the current state     
//}
//
//MenuState menu_handle_start_walkup_menu(MenuContext& ctx)
//{
//    ///////////////////////////////////////////
//    ////////////// Walkup Menu ///////////////
//    ///////////////////////////////////////////
//    wipeOLED();
//    u8x8.setCursor(0, 2);
//    u8x8.print("Walkup Dur: "); 
//    u8x8.print(ctx.menuWalkup == 10 ? 10 : 20);  
//    doButtonMenu();
//    
//    switch (waitButton())
//    {
//      case BUTTON1: 
//      paramStore.walkUp = ctx.menuWalkup;
//      return MenuState::menu_root;
//  
//    case BUTTON2: 
//      ctx.menuWalkup = ctx.menuWalkup == 10 ? 20 : 10;
//      break;
//  
//    case BUTTON3: 
//      ctx.menuWalkup = ctx.menuWalkup == 10 ? 20 : 10;
//      break;
//  
//    case BUTTON4:
//      ctx.menuWalkup = paramStore.walkUp;
//      return MenuState::menu_root;
//
//    }
//  
//    return MenuState::start_walkup_menu; // by default always return the current state    
//}
//
//MenuState menu_handle_start_finals_menu(MenuContext& ctx)
//{
//  ///////////////////////////////////////////
//    ////////////// Finals Menu ////////////////
//    ///////////////////////////////////////////
//
//    wipeOLED();
//
//    // loop on "Finals" selection 
//    for (bool selected = false; !selected; )
//    {
//      u8x8.setCursor(0, 2);
//      u8x8.print("Finals: "); 
//      u8x8.print(ctx.menuFinals == 0 ? "No " : "Yes");   
//    
//      switch (waitButton())
//      {
//      case BUTTON1: 
//        paramStore.isFinals = ctx.menuFinals;
//        paramStore.notFlint = true;
//        selected = true;
//        break;        
//      
//      case BUTTON2: 
//      case BUTTON3: 
//        ctx.menuFinals = ctx.menuFinals == 0 ? 1 : 0;
//        break;
//      
//      case BUTTON4:
//        ctx.menuFinals = paramStore.isFinals;
//        if (ctx.menuFinals == 0)
//        {         
//          return MenuState::menu_root;
//        }
//        selected = true;
//        break;
//      }
//    }
//    
//    u8x8.setCursor(0, 2);
//    u8x8.print("Finals: "); 
//    u8x8.print(ctx.menuFinals == 0 ? "No " : "Yes");
//    
//    if (ctx.menuFinals)
//    {
//      paramStore.notFlint = true;
//      paramStore.Details = 1;                           //  Single detail by default
//      paramStore.startCountsIndex = 4;                  //  set to 20sec
//      paramStore.maxPrac = 0;                           //  no practice
//      paramStore.maxEnds = 5;                           //  5 ends
//      
//      for (;;)
//      {
//        u8x8.setCursor(0, 4);
//        u8x8.print("Alternating: ");
//        u8x8.print(ctx.menuAlternating == 0 ? "No " : "Yes");
//        doButtonMenu();
//        
//        switch (waitButton())
//        {
//        case BUTTON1: 
//          paramStore.isAlternating = ctx.menuAlternating;
//          return MenuState::start;
//        
//        case BUTTON2: 
//          ctx.menuAlternating = ctx.menuAlternating == 0 ? 1 : 0;
//          break;
//        
//        case BUTTON3: 
//          ctx.menuAlternating = ctx.menuAlternating == 0 ? 1 : 0;
//          break;
//        
//        case BUTTON4:
//          ctx.menuAlternating = paramStore.isAlternating;
//          return MenuState::menu_root;     
//        }
//      }
//    }
//    
//    doButtonMenu();
//    
//    switch (waitButton())
//    {
//    case BUTTON1: 
//      paramStore.isFinals = ctx.menuFinals;
//      paramStore.notFlint = true;
//      return MenuState::start;
//
//    case BUTTON2: 
//    case BUTTON3: 
//      ctx.menuFinals = ctx.menuFinals == 0 ? 1 : 0;
//      break;
//
//    case BUTTON4:
//      ctx.menuFinals = paramStore.isFinals;
//      return MenuState::menu_root;
//    }
//    
//    return MenuState::start_finals_menu; // by default always return the current state    
//}
//
//
//
//int goMenu(bool displayParams)
//{
//  MenuContext ctx;
//
//  // handler functions will only have the context, so copy the argument value into it
//  ctx.displayParams = displayParams; 
//  
//  for (
//    MenuState state = MenuState::start;  // Creates a particular instance of the state, default state is "started"
//    state != MenuState::confirmed;  // keep looping until state reaches "confirmed" 
//  )
//  {
//    switch (state)
//    {
//    case MenuState::start:                    state = menu_handle_start(ctx); break;
//    case MenuState::awaiting_confirmation_1:  state = menu_handle_awaiting_confirmation_1(ctx); break;
//    case MenuState::awaiting_confirmation_2:  state = menu_handle_awaiting_confirmation_2(ctx); break;
//    case MenuState::menu_root:                state = menu_handle_menu_root(ctx); break;
//    case MenuState::start_INTERVALS_menu:     state = menu_handle_start_INTERVALS_menu(ctx); break;
//    case MenuState::start_duration_menu:      state = menu_handle_start_duration_menu(ctx); break;
//    case MenuState::start_ends_menu:          state = menu_handle_start_ends_menu(ctx); break;
//    case MenuState::start_practice_menu:      state = menu_handle_start_practice_menu(ctx); break;
//    case MenuState::start_details_menu:       state = menu_handle_start_details_menu(ctx); break;
//    case MenuState::start_walkup_menu:        state = menu_handle_start_walkup_menu(ctx); break;
//    case MenuState::start_finals_menu:        state = menu_handle_start_finals_menu(ctx); break;
//    
//    case MenuState::confirmed:          break; // only handling to avoid the compiler warning (that one of the enum values is not handled)
//    }
//  }
//  
//  
//  // we are out of the loop as we've reached 'confirmed' state 
//  clearFromLine(5); 
//  u8x8.draw2x2String(0, 6, "..WAIT..");
//  intervalOn = false;
//  started = false;
//  delay(250);
//  
//  return 1;
//}
