...
void writeOLED_Data(uint8_t archerIndex, byte nID){
  ...
  if (p_Store.Details == 2){                                        // Double detail?
    ...
    if (!p_Store.isFinals && !p_Store.ifaaIndoor){
      ...
      u8x8.print(sEcount <= 6 ?   (sE_iter%2 == 1 ? "AB" : "CD"):     // if 1st first half of IFAA round
...
void setHeader(void){
  u8x8.setCursor(0,0);
  u8x8.inverse();
  if (!p_Store.teamPlay || !op_Chan) u8x8.print(" Time Controller"); // op_Chan refers to the operational channel in 2ch team play
  else {  
    char buf[4];
