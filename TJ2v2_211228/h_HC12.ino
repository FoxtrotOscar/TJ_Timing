...
bool newChannel(int newChan, bool alterGlobally){
  ...
  pauseMe(100);                                  // 80 ms delay before operation per datasheet
  tmp_str[4] = '0';                             // |
  byte x = newChan / 10;                        // |
  ...
  if (alterGlobally){                           //   Do this to both LOCAL & any REMOTE listening?
    ...
    HC12.println(tmp_str);                      // Send command to REMOTE HC12
    pauseMe(500);                              // Wait 0.5s for a response
  }
  digitalWrite(HC12SetPin, LOW);                // Now enter COMMAND mode locally
  pauseMe(100);
  HC12.println(tmp_str);                        // Send command to local HC12
  ...
  //currBaud = commandBaudRate(false);                     // return to TRANSPARENTmode rate
  digitalWrite(HC12SetPin, HIGH);               // Exit command & enter transparent mode
  pauseMe(800);                                 // Delay before proceeding to allow HC12 resetting
  HC12ReadBuffer = "";
  return localSuccess;
}
