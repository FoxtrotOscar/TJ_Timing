...
bool goEmergencyButton(uint8_t AIndex){
  ...
  if (readButtons() == BUTTON4) {                           // Handle Red-button push
    ...
    u8x8.print("STOP");
    delay(5*tick);
    n = handleEmergencyRestart();                           // go fetch the next step decision
    if (AIndex < 3){    
      displayParamsOnOLED();
      if (n == startCounts[paramStore.startCountsIndex]) {
        doBarCount(AIndex);
        writeOLED_Data(1);
      } else if (n != 0){
        goWhistle(1);
...
int16_t handleEmergencyRestart(void){
  ...
  switch (waitButton()) {
    ...
    case BUTTON1: 
      n = (n+11 > maxPossSecs) ? maxPossSecs : n+11;        // rewind 10 sec and continue
      clearMatrix();
    break;
    
    case BUTTON2:
      reStartEnd = true;                                    //  
      n = startCounts[paramStore.startCountsIndex];         // restart this end, continue with competition
      clearMatrix(); //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    break;
