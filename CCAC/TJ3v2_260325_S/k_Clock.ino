...
//  BTN4 detection inside the do{}while loop.
      ...
      if (readButtonNoDelay(button4Pin) == 1) {            // still held
        ...
        for (;;) {
          ...
          if (btn == BUTTON1) {
            clearMatrix(false);
            displayParamsOnOLED();
            sEcount--;              // compensate — writeReady expects pre-increment value
            writeReady();           // now shows correct distance
            sEcount++;              // restore
            writeMenuCommands();

            for (;;) {                     // second BTN1 confirms proceed
                if (readButtons() == BUTTON1) break;
            }
            reStartEnd = true;
            return;
          }
