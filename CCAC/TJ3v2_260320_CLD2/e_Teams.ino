...
void goTeamPlay(byte teamType) {                // teamType is p_Store.teamPlay val 0-4, Zero is Off
  ...
  uint8_t tempArrowCount[3];                    // moved to function scope for snapshot access
  sEcount               = 0;
  startOver             = false;
  continueOn            = true;
  ...
  while (continueOn && !startOver) {
    while (sEcount < p_Store.maxEnds || shootOff) {
      ...
      nID = !shootOff ? goChooseArcher() : firstToShoot;                      // team that shot first shall start shooting
      if (sEcount) clearAB(nID, false); 
      nID == 1 ? set_A(1) : set_B(2);                                         // set nID to match first team up
      pauseMe(20);
      if (!sEcount) firstToShoot = nID;                                       // save first team up for tie-breaks
      sEcount++;
       ...
       *  start the first set (three arrows each), with NO whistle in between
       *  each arrow except where time is over-run - then whistle x1.  
       *  As usual, whistle x3 at the end of each set. 
       *  Judge progresses play with green button; if not pressed and zero 
       *  reached then there is a whistle.
       *  We decide the first archer at the head of each three arrows, put up
       *  a signal for "Scoring", followed by "Ready" on green button.
       *  Green Button re-commences.
       */

      writeOLED_Data(nID, nID);                                               // set up for ends of 4 or 6 arrows
      // Re-initialise tempArrowCount on each pass as original declaration did
      tempArrowCount[0] = 0;
      tempArrowCount[1] = arrowCount;
      tempArrowCount[2] = arrowCount;
      if (shootOff) tempArrowCount[1] = tempArrowCount[2] = set_size;        // shootoff overrides arrow count

      // Snapshot all Teams-local state immediately before entering countdown
      // Used to restore cleanly if operator chooses to reconsider emergency action
      byte    snapNID_t        = nID;
      uint8_t snapArrowCount_t = arrowCount;
      bool    snapShootOff_t   = shootOff;
      byte    snapFirstToShoot = firstToShoot;
      uint8_t snapWhistleCount = whistleCount;
      uint8_t snapTempAC[3];
      memcpy(snapTempAC, tempArrowCount, sizeof(tempArrowCount));             // deep copy arrow count array

      while (n_Count_[nID] > 0) {                                            // repeat until count exhausted
        if (!goEmergencyButton(nID, nID)) {
          // Normal run — no emergency
          writeStopwatch(n_Count_[nID]);
          HC12.print(F("font 13\r"));
          goClock(offSet, nID);                                               // handles display formatting
          ...
          if (flipFlag) {                                                     // detail change invoked
            if (whistleCount++ <= set_size) goWhistle(1);                    // skip 1x whistle near 3x whistle
            shootOff ?
                tempArrowCount[nID]-- :                                       // shootoff: reduce arrowcount
                tempArrowCount[nID] -= set_size;                              // or reduce by arrows per set
            tempArrowCount[nID] == 0 ? n_Count_[nID] = 0 : n_Count_[nID] += 1; // keep screen count right
            nID = (nID == 1 ? set_B(1) : set_A(2));                          // hold curr count and change screen
            pauseMe(50);
            setHeader();                                                      // write TimeController or Ch no.
            writeOLED_Data(nID, nID);
          }
          flipFlag = handleCount(secCount, nID);                              // false if count didn't run out

        } else {
          // Returning from emergency handling
          if (emergencyReconsider) {
            // Operator reconsidered — restore all Teams-local state to
            // pre-emergency snapshot. Global state (sEcount, sE_iter,
            // shootDetail) already restored inside goEmergencyButton.
            nID          = snapNID_t;
            arrowCount   = snapArrowCount_t;
            shootOff     = snapShootOff_t;
            firstToShoot = snapFirstToShoot;
            whistleCount = snapWhistleCount;
            memcpy(tempArrowCount, snapTempAC, sizeof(tempArrowCount));       // restore arrow count array
          }
          // Re-sync display and timer on return from emergency regardless
          // of whether reconsider was chosen or a decision was made
          for (;;) {
            byte btn = readButtons();
            if (btn == BUTTON1) break;                                        // wait for operator to proceed
          }
        }
      }

      goWhistle(3);                                                           // end of set whistle
      whistleCount = 0;                                                       // reset whistle counter for next set
      clearAB(nID, true);                                                     // clear matrix with Score and Collect
      shootOff = false;
    }                                                                         
