...
void writeHalt(void){
  ...
  pauseMe(10);
  clearFromLine(5); 
  disp.draw2x2String(0, 6, "..HALT..");
  writeRectangle( 1, 0, 31, 64, 32);                                  // fill screen with red
  pauseMe(3*tick);
  score_Collect(false);                                               // show a red screen, then Score & Collect
  pauseMe(3*tick);
  clearMatrix(false);  //cd  poss bug origin
  ...
  if (p_Store.Details == 2){
    if (sEcount < p_Store.maxEnds) {                                // ???
      ...
      if (p_Store.isFinals 
        ...
        delay(tick); //}}}}}}}}}}}}}}}}}}}}}}}
        sendSerialS( orange, /*column=*/ 8, /*line=*/ lnNumber, "FINALS");
        delay(tick);
      }else{
        sendSerialS ( orange, /*column=*/ 24, /*line=*/ lnNumber, 
                    ( !p_Store.ifaaIndoor ?
                    ( sE_iter%4 == 2 ? "C D" : "A B") :
                    ( sEcount  < 6 ? 
                    ( sE_iter%2 == 0 ? "A B" : "C D") : 
...
void writeReady(void){ 
  ...
  sendSerialS( orange, /*column=*/ 1, /*line=*/ 30, i0,i1);
}

void writeReadySet(void){
  clearMatrix(false);
  sendSerialS(green, 0, 15, "   READY");
  sendSerialS(green, 0, 29, "   SET:");
  sendNumber(green, 44 , 29, sEcount);
}
...
void doCountdownBar(void){
  byte loc_Count = (p_Store.isFlint) && (sEcount > 7) ?             // If a Flint and also a Flint walkup round, 
                    20 :  p_Store.walkUp;                           // determine the starting count value @20sec
  byte n_Loc = loc_Count;                                           // for the actual count @20sec
  byte lnNumber = 30;
  int clockPulse;
  byte rectWide = 49;     
  writeRectangle(red, 0, lnNumber, rectWide, 13 );                    // rCol, rX0, rY0, rWid, rHi
  HC12.flush();
  ...
  do{                                                               // : run a smooth countdown bar
    ...
    switch (abs(clockPulse)) {
      ...
      case 0:
        ...
        n_Loc--; 
        HC12.flush();
        if (n_Loc >=10) break;                                      // where count is greater than 10(flint), skip forward
...
void sendDetail(bool pos_High) {
  ...
  if (p_Store.Details == 2 && !p_Store.ifaaIndoor) {                // |Double detail, alternating?
    if (!shootDetail && (sE_iter % 4 == 1)) {                       // |shootDetail 0 == AB, 1 == CD
      writeA_B(false);                                              // |
    } else {                                                        // |
      if (shootDetail  && (sE_iter % 4 == 2)) {                     // |If not IFAA round and double detail
        writeC_D(false);                                            // |
      } else {                                                      // |if so, set the correct
        if (!shootDetail && (sE_iter % 4 == 3)) {                   // |detail identifier on the screen
          /*countPractice ? writeA_B(false) :*/ writeC_D(pos_High);       // |
        } else {                                                    // |
          /*countPractice ? writeC_D(false) :*/ writeA_B(pos_High);        // |
        }
      }
    }
  } else if (p_Store.Details == 2 && p_Store.ifaaIndoor) {
     printDebugLine(false, __LINE__, __NAME__);
    sEcount  <= 6  ?  (sE_iter % 2 == 0 ? writeC_D(true)  : writeA_B(false)) :
                      (sE_iter % 2 == 1 ? writeC_D(false) : writeA_B(true)) ;

  } else if(p_Store.ifaaIndoor) writeA_B(pos_High);
  HC12.flush();
}


void writeA_B(bool pos_High) {
  HC12.print(F("font 9\r"));                                        // font 12 high double
  HC12.flush();
  lnNumber = (pos_High ? 12 : 30);                                    // Write A & B (or B and A for IFAA pt 2)
  // p_Store.ifaaIndoor && sEcount >=7 ? sendSerialS(2, 1,  lnNumber, "B") :
  //                                     sendSerialS(2, 1,  lnNumber, "A") ;
  // p_Store.ifaaIndoor && sEcount >=7 ? sendSerialS(2, 56, lnNumber, "A") :
  //                                     sendSerialS(2, 56, lnNumber, "B") ;
  sendSerialS(2, 1,  lnNumber, (p_Store.ifaaIndoor && sEcount >=7 ? "B" : "A")) ;
  sendSerialS(2, 56, lnNumber, (p_Store.ifaaIndoor && sEcount >=7 ? "A" : "B")) ;
  HC12.flush();
}

void writeC_D(bool pos_High) {
  HC12.print(F("font 9\r"));
  HC12.flush();
  lnNumber = (pos_High ? 12 : 30);
  // p_Store.ifaaIndoor && sEcount >=7 ? sendSerialS(2, 1,  lnNumber, "D") :
  //                                     sendSerialS(2, 1,  lnNumber, "C") ;
  // p_Store.ifaaIndoor && sEcount >=7 ? sendSerialS(2, 56, lnNumber, "C") :
  //                                     sendSerialS(2, 56, lnNumber, "D") ;
  sendSerialS(2, 1,  lnNumber, (p_Store.ifaaIndoor && sEcount >=7 ? "D" : "C")) ;
  sendSerialS(2, 56, lnNumber, (p_Store.ifaaIndoor && sEcount >=7 ? "C" : "D")) ;
