...
int16_t handleEmergencyRestart_Screen2(byte nID) {
  int8_t  savedIter   = sE_iter;          // snapshot active sub-phase
  uint8_t maxPossSecs = startCounts[p_Store.startCountsIndex];

  for (;;) {
    clearFromLine(1);

    // Context: end number and active sub-phase highlighted
    disp.setCursor(0, 1);
    ...
    disp.setFont(u8x8_font_5x7_f);
    disp.noInverse();
    disp.print("(from active grp)");
    disp.setFont(u8x8_font_chroma48medium8_r);

    switch (waitButton()) {

      case BUTTON1:
        // Restart full end from first sub-phase
        reStartEnd = true;
        for (byte reset = 0; reset <= 2; reset++)
          n_Count_[reset] = maxPossSecs;
      ...
      case BUTTON2:
        // Resume from active sub-phase — restore snapshot
        reStartEnd = true;
        for (byte reset = 0; reset <= 2; reset++)
          n_Count_[reset] = maxPossSecs;
        sE_iter     = savedIter;          // explicit restore for clarity
        shootDetail = p_Store.ifaaIndoor
                      ? (sEcount > 6 ? true : false)
                      : ((sEcount % 2) == 1 ? false : true);
        clearMatrix(false);
      ...
      case BUTTON4:
        return -2;                        // reconsider — propagates to goEmergencyButton
    }
  }
}
...
int16_t handleEmergencyRestart(byte nID) {
  ...
  for (;;) {                              // loops on BTN3 back from screen 2
    ...
    // Context line — operator always knows where they are
    disp.setCursor(0, 1);
    disp.setFont(u8x8_font_5x7_f);
    ...
    // BTN2 — restart options (goes to screen 2)
    disp.setCursor(0, 4);
    disp.setFont(u8x8_font_chroma48medium8_r);
    disp.inverse();
    ...
    switch (waitButton()) {
      ...
      case BUTTON1:
        // Step back 10s — sEcount and sE_iter correct as-is
        n_Count_[nID] = (n_Count_[nID] + 11 > maxPossSecs)
                        ? maxPossSecs
                        : n_Count_[nID] + 11;
        clearMatrix(false);
        return n_Count_[nID];
      ...
      case BUTTON2: {
        ...
        if (result == -1) continue;       // BTN3 on screen 2 — back to screen 1
      ...
      case BUTTON4:
        // Reconsider — signal goEmergencyButton to restore snapshot
        return -2;
    }
  }
}
...
// ----------------------------------------------------------------
//  goEmergencyButton()
//  Called from the main run loops on BUTTON4 detection.
//  Snapshots state at moment of emergency so BTN4 reconsider
//  can restore cleanly regardless of how many times it is used.
// ----------------------------------------------------------------
bool goEmergencyButton(uint8_t AIndex, byte nID) {
  ...
  if (readButtons() == BUTTON4) {
    ...
    for (;;) {
      ...
      if (result == -2) {
        // Operator reconsiders — restore global snapshot
        sE_iter     = snapIter;
        sEcount     = snapEcount;
        shootDetail = snapDetail;
        emergencyReconsider = true;     // signal to calling mode
        clearFromLine(1);
        disp.setCursor(0, 3);
        disp.print("  Reconsidering ");
        disp.setCursor(0, 5);
        disp.print(" BTN[4] to retry");
        pauseMe(2 * tick);
        emergencyReconsider = false;    // clear before re-entering menu
        continue;
      }

      // Decision made
      n_Count_[nID] = result;
      emergencyReconsider = false;      // ensure clean on exit
      break;
    }
...
//   // --- Context line: tell operator exactly where they are ---
//   disp.setCursor(0, 1);
//   disp.setFont(u8x8_font_5x7_f);
//   if (p_Store.ifaaIndoor) {
//     disp.print(sEcount <= 6 ? "IFAA ends 1-6 AB" : "IFAA ends 7+  DC");
//   } else if (p_Store.isFlint && sEcount > 6) {
//     disp.print("Flint walkup end");
//   }
//   disp.setFont(u8x8_font_chroma48medium8_r);
...
//   if (p_Store.ifaaIndoor)
//     disp.print(sEcount <= 6 ? "(restart: AB 1st)" : "(restart: DC 1st)");
//   else
//     disp.print("(recommence end )");
//   disp.setCursor(0, 6);
//   disp.setFont(u8x8_font_chroma48medium8_r);
...
//     case BUTTON1: 
//       // Step back 10s — no state change needed, sEcount/sE_iter stay correct
//       n_Count_[nID] = (n_Count_[nID] + 11 > maxPossSecs) 
//                       ? maxPossSecs 
//                       : n_Count_[nID] + 11;
//       clearMatrix(false);
...
//     case BUTTON2:
//       // Restart current end from zero
//       reStartEnd = true;
//       for (byte reset = 0; reset <= 2; reset++)
//         n_Count_[reset] = startCounts[p_Store.startCountsIndex];
      
//       // KEY FIX: reset sE_iter to correct parity for THIS end's first sub-phase
//       // sEcount is unchanged — we are restarting the same end
//       sE_iter = deriveIterForEnd(sEcount) - 1;  
//       // subtract 1 because goNormal_Op does sE_iter += 1 at top of its loop
//       // so on re-entry it will immediately become deriveIterForEnd(sEcount)
      
//       // shootDetail: re-derive for this end to be safe
//       shootDetail = p_Store.ifaaIndoor 
//                     ? (sEcount > 6 ? true : false) 
//                     : ((sEcount % 2) == 1 ? false : true);
...
//       startOver = true;
//       memset(n_Count_, 0, sizeof(n_Count_));
//       // sEcount and sE_iter will be reset in loop() → zeroSettings() path ✓
//       break;
//   }
//   return n_Count_[nID];
// }
