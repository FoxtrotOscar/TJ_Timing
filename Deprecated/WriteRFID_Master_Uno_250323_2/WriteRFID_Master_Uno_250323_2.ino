...
// CCAC
const byte   Key12 = 155;
const byte   Key13 = 136;
const byte   Key14 = 220;
const byte /*const uint8_t*/   Key15 =  28;
...
const int button1Pin  = 7;                               // the number of the pushbutton pin UNO
  ...
  for (;;){
    ...
    if (btn == BUTTON1){
      ...
      do {
        ...
        if (paramIndex == paramShow+1) {                                     // catch BUTTON4 exit
          break;
        } else {
      ...
      } while ((waitButton() != BUTTON4) || (paramIndex == 13));
      printDebugLine(__LINE__, __NAME__);
      clearFromLine(1);
      printDebugLine(__LINE__, __NAME__); 
