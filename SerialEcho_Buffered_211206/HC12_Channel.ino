...
void showParam(char newParam){
  digitalWrite    (setupPin,    LOW);            // PC15: set active
...
void commandBaudRate(bool command){
  command ? digitalWrite(setupPin, LOW) : digitalWrite(setupPin, HIGH);
  pauseMe(800);
  HC12.flush();
  HC12.end();
  pauseMe(100);
  HC12.begin((command ? 9600 : 2400), SERIAL_8N1);
  pauseMe(500);
}
