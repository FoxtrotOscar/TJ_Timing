...
#ifdef DEBUG
constexpr uint16_t tick   = 1000;                  // short "second" interval <<(nominally 1000)>>
#else
...
void setup() {
  ...
  Serial.print("Here with currBaud == ");
  Serial.println(currBaud);
  ...
  */
  ...
  u8x8.draw2x2String(0, 6, "..WAIT..");
  writeSplash(false);                             // write the splash without the scroll
  pauseMe(2 * tick);
